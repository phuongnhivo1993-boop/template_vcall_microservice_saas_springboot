# VCall Contact Center - System Overview
...
| Domain | Service | Port | Database | Description |
|--------|---------|------|----------|-------------|
...
```bash
make infra-up
# Includes: PostgreSQL, Redis, Kafka, ElasticSearch, MinIO, ClickHouse
```
...
# 4. Microservices (can start in parallel)
...
## API Documentation
After starting API Gateway (port 8080):
...
The build.sh script:
1. Checks Java version (requires 21+)
2. Checks Maven availability
3. Builds common module first
4. Builds all services with tests skipped

## Known Issues (as of build attempt)
- RecordingService had duplicate @GetMapping annotations and malformed catch statements
- OmnichannelService missing UUID import
- TicketService using deprecated Spring Test annotations (MockitoBean)